...
# if(!require(IDentifyEngine)){
	devtools::install_bitbucket("Molaison/MantaID",upgrade = "never")
# }
# for(pac in packages){
...
```{r}
attributes = mi_get_ID_attr(biomart = "genes", dataset = "hsapiens_gene_ensembl")
flt_attri = attributes %>% slice(1,2,6,7)
data_ID = mi_get_ID(flt_attri,biomart = "genes", dataset = "hsapiens_gene_ensembl")
data_ID
```
...
result <- mi_run_bmr(train, row_num = 4000) 
...
library(reticulate)
path_to_python <- use_python(python = "D:\\Softwares\\Coding\\Python3.7.6\\python.exe")
library(tensorflow)
